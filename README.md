...
2. Create a MariaDB container with host volume:
  ...
  *Note:* You need to give the container a name in order for Ghost to resolve to your hostname.
...
- The ghost container has been migrated to a non-root container approach. Previously the container run as `root` user and the ghost daemon was started as `ghost` user. From now on, both the container and the ghost daemon run as user `1001`.
